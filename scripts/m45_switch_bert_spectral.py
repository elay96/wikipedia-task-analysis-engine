#!/usr/bin/env python3
"""
M45: Switch Rate - BERT Embeddings + Spectral Embedding + K-means (k=5)
=======================================================================
Per page: assign cluster from BERT Spectral+K-means model (M42).
...
MODEL_PATH = DATA_DIR / 'cleaned' / 'bertopic_spectral.json'
...
def plot_domain(domain, participants_data, output_path):
    ...
    fig.suptitle(
        f'M45: Switch Rate (BERT Spectral+K-means, k=5) - {domain}',
...
def main():
    ...
    print("[M45] Switch Rate - BERT Spectral+K-means (k=5)")
    ...
    for domain in sorted(domain_data.keys()):
        plot_domain(domain, domain_data[domain], OUTPUT_DIR / f'm45_switch_bert_spectral_{domain}.png')

    df_out = pd.DataFrame(rows_csv)
    csv_path = OUTPUT_DIR / 'm45_switch_bert_spectral.csv'
