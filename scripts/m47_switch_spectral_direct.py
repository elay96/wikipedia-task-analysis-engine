#!/usr/bin/env python3
"""
M47: Switch Rate - Spectral Embedding + K-means on Raw BERT (no UMAP)
======================================================================
Per page: assign cluster from Spectral+K-means model (M46).
...
def plot_domain(domain, participants_data, output_path):
    ...
    fig.suptitle(
        f'M47: Switch Rate (Spectral+K-means on raw BERT, k=3) - {domain}',
...
def main():
    ...
    print("[M47] Switch Rate - Spectral+K-means on raw BERT (no UMAP)")
    ...
    print(f"  Method: {model['method']}, silhouette: {model['silhouette_score']:.3f}")
    ...
    for domain in sorted(domain_data.keys()):
        plot_domain(domain, domain_data[domain], OUTPUT_DIR / f'm47_switch_spectral_direct_{domain}.png')

    df_out = pd.DataFrame(rows_csv)
    csv_path = OUTPUT_DIR / 'm47_switch_spectral_direct.csv'
