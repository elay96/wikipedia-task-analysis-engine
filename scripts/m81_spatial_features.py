#!/usr/bin/env python3
"""
M81: Re-run of M74 spatial-feature extraction using the new GBM tagging algorithm.
=================================================================================
Same pipeline as M76 (M74-style 30 features per pid, but mode tagging from the
M75e-calibrated GBM instead of Enter_*_mode events). Refit + apply, fresh outputs.
...
Outputs:
  output/m81_per_trial_features.csv        660 rows
  output/m81_spatial_features.csv          132 rows x 30 features
  output/m81_spatial_maps.pdf              one page per participant
  docs/m81_model_explanation.html          model logic + accuracy report
"""
...
PER_TRIAL_OUT = OUTPUT_DIR / 'm81_per_trial_features.csv'
PER_PID_OUT = OUTPUT_DIR / 'm81_spatial_features.csv'
PDF_OUT = OUTPUT_DIR / 'm81_spatial_maps.pdf'
HTML_OUT = DOCS_DIR / 'm81_model_explanation.html'
...
def plot_participant_page(pdf, pid, condition, trials_data, x_range, y_range, summary_rows):
    ...
    fig.suptitle(f'Participant {pid}  |  Condition: {condition}  |  '
                 f'Total collected: {total}  (M81: GBM-classified modes)',
...
<head>
<meta charset="UTF-8">
<title>M81 - Mode classifier explanation</title>
<style>
...
<body>
<h1>M81 - מסווג מצב Explore/Exploit (הסבר המודל)</h1>
<div class="meta">
  Script: <code>scripts/m81_spatial_features.py</code> &middot;
...
<p>
  ...
  כלומר ה-Enter events כמעט לא משקפים את ההתנהגות בפועל. M81 מחליף את זה
...
<table>
  ...
  <tr><td><strong>M81: GBM (17 features) + smoothing 7s + tuned threshold</strong></td>
...
<table>
  ...
  <tr><td><code>output/m81_per_trial_features.csv</code></td>
      <td>660 שורות (132 pids × 5 trials), 15 פיצ'רים מבוססי המודל</td></tr>
  <tr><td><code>output/m81_spatial_features.csv</code></td>
      <td>132 שורות, 30 עמודות (mean + sd לכל פיצ'ר)</td></tr>
  <tr><td><code>output/m81_spatial_maps.pdf</code></td>
