#!/usr/bin/env python3
"""
M43: Switch Rate - BERT Embeddings + K-means (k=5)
===================================================
Per page: assign cluster from BERT K-means model (M42).
...
BERT_KMEANS_PATH = DATA_DIR / 'cleaned' / 'bertopic_kmeans.json'
...
def load_bert_kmeans_topics():
    with open(BERT_KMEANS_PATH, 'r') as f:
        bm = json.load(f)
...
def plot_domain(domain, participants_data, output_path):
    ...
    fig.suptitle(
        f'M43: Switch Rate (BERT K-means, k=5) - {domain}',
    ...
    for i, (pid, topics, sr) in enumerate(participants_data):
        ...
        for t in unique_topics:
            ...
            label = f'T{t}' if t >= 0 else 'unknown'
...
def main():
    ...
    print("[M43] Switch Rate - BERT K-means (k=5)")

    trials = load_trials()
    pids, pid_trials = get_pids_and_trials(trials)
    print(f"  {len(trials)} trials from {len(pids)} participants")

    print("  Loading BERT K-means model...")
    topic_assignments = load_bert_kmeans_topics()
    print(f"  {len(topic_assignments)} article assignments")
    ...
    for domain in sorted(domain_data.keys()):
        output_path = OUTPUT_DIR / f'm43_switch_bert_kmeans_{domain}.png'
        plot_domain(domain, domain_data[domain], output_path)

    df_out = pd.DataFrame(rows_csv)
    csv_path = OUTPUT_DIR / 'm43_switch_bert_kmeans.csv'
