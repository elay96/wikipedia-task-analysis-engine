...
def build_sequences_time_median(pids, pid_trials, slug_idx, pc, domain_medians):
    """Build sequences using per-trial time median for pages, domain median for transitions."""
...
def main():
    ...
    pid_data = build_sequences_time_median(pids, pid_trials, slug_idx, lsa, domain_medians)

    plot_trial_grid_with_switches(pid_data, pids, 0, var_explained, n_components, 'm27_trial1.png')
    plot_trial_grid_with_switches(pid_data, pids, 1, var_explained, n_components, 'm27_trial2.png')
    ...
    for t_idx, t_name in [(0, 'Trial 1'), (1, 'Trial 2')]:
        ...
        print(f'\n=== M27 {t_name} (LSA {n_components}D, time-median pages + cross-subj median transitions) ===')
